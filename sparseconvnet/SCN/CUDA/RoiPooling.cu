...
__global__ void RoiPooling_fp(T *input_features, T *output_features,
...
template <typename T>
void cuda_RoiPooling_ForwardPass(T *input_features, T *output_features,
                                 Int nPlanes, Int input_stride,
                                 Int output_stride, RuleBook _rules) {
  RULEBOOKITERATOR((RoiPooling_fp<T, 32, 32><<<32, dim3(32, 32)>>>(
      input_features, output_features, nPlanes, input_stride, output_stride,
      rbB, nHotB));
                   , )
}
template <typename T, Int NTX, Int NTY>
__global__ void RoiPooling_bp(T *input_features, T *d_input_features,
...
template <typename T>
void cuda_RoiPooling_BackwardPass(T *input_features, T *d_input_features,
                                  T *output_features, T *d_output_features,
                                  Int nPlanes, Int input_stride,
                                  Int output_stride, RuleBook _rules) {
  RULEBOOKITERATOR((RoiPooling_bp<T, 32, 32><<<32, dim3(32, 32)>>>(
